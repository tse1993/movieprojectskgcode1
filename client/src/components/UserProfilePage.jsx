...
import { ArrowLeft, User, Star, Heart, Bookmark, Calendar, Filter, GridIcon, List, Users, Play, Trash2, TrendingUp, Award, Clock } from "lucide-react";
...
export default function UserProfilePage(props) {
    ...
    // Transform favoriteMovies data into FavoriteMovie-like objects
const favoriteMoviesList = favoriteMovies
  .map((favorite) => {
    ...
    return {
      ...
      // στο αρχικό model έχεις single genre -> το κάνουμε array
      genre: [movie.genre],
      rating: movie.rating,
      // placeholders επειδή δεν υπάρχουν στο movie model
      runtime: 120,
  ...
  // Transform movieRatings into RatedMovie-like objects
  const ratedMovies = movieRatings
    ...
    // Transform watchlistMovies into detailed format
    ...
    const allGenres = Array.from(
      new Set((favoriteMoviesList || []).flatMap((movie) => movie.genre || []))
    );

    // Filter and sort movies
    let filteredMovies = favoriteMoviesList; // ή: let filteredMovies = [...favoriteMoviesList];

    // Filter by genre
    if (filterGenre !== "all") {
      filteredMovies = filteredMovies.filter((movie) =>
        (movie.genre || []).some(
          (genre) => genre.toLowerCase() === filterGenre.toLowerCase()
        )
      );
    }
    ...
    if (filterRating !== "all") {
      ...
      filteredRatedMovies = filteredRatedMovies.filter(
        (movie) => movie.userRating >= minRating && movie.userRating < minRating + 2
      );
    }
    ...
    // Average user rating
    const averageUserRating = ratedMovies.length
      ? ratedMovies.reduce((sum, movie) => sum + movie.userRating, 0) /
        ratedMovies.length
      : 0;
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-2 p-3 rounded-lg bg-muted/50">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{favoriteMoviesList.length}</p>
                      </div>
                    </div>
                  ...
                  {filteredMovies.length === 0 ? (
                    <div className="py-8 text-center">
                      ...
                      <Button onClick={onBack} size="sm">Browse Movies</Button>
                    </div>
                  ) : viewMode === 'grid' ? (
                            ...
                            <div className="flex-1 space-y-1">
                              <div className="flex items-start justify-between">
                                <div>
                                  ...
                                  <p className="text-xs text-muted-foreground">{movie.year} • {formatRuntime(movie.runtime)}</p>
                                </div>
                                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
                    <div className="flex gap-4 flex-wrap">
                      <div className="flex items-center space-x-2 p-3 rounded-lg bg-muted/50">
                        ...
                        <div>
                          <p className="text-sm text-muted-foreground">Movies Rated</p>
                          <p className="font-semibold">{ratedMovies.length}</p>
                        </div>
                      </div>
                              ...
                              <div className="h-8 bg-muted rounded flex items-end justify-center">
                                {count > 0 && (
                                  <div 
                                    ...
                                    style={{ height: `${Math.max((count / Math.max(...ratingDistribution.map(r => r.count))) * 100, 10)}%` }}
                                  />
                                )}
                              </div>
                      ...
                      {filteredRatedMovies.map((movie) => (
                        <Card key={movie.id} className="group cursor-pointer hover:shadow-lg transition-all duration-300">
                          ...
                          <CardContent className="p-3">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Rated {formatDate(movie.ratedDate)}
                            </p>
                          </CardContent>
                        </Card>
                      ))}
                  ...
                  {watchlistMoviesWithDetails.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {watchlistMoviesWithDetails
                        .filter(Boolean) // πετάει null/undefined
                        .map((movie) => (
